            ...
            <div className="p-3 sm:p-6 max-w-4xl mx-auto space-y-6 animate-in fade-in duration-500">
                <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-white font-bold text-sm sm:text-base">Recent Alerts</span>
                    </div>
                    {notifications.length > 0 && (
                        <div className="flex items-center gap-2 sm:gap-4">
                            <Button
                                ...
                                size="sm"
                                onClick={clearAll}
                                className="text-[10px] sm:text-xs text-white/40 hover:text-destructive hover:bg-destructive/10 h-8 font-bold px-2 sm:px-3"
                            >
                ...
                <div className="grid gap-4">
                    {notifications.map((notif) => (
                        <div
                            key={notif.id}
                            className={cn(
                                "group relative bg-[#121212] border border-white/5 hover:border-white/10 rounded-2xl p-4 sm:p-6 transition-all duration-300 shadow-lg",
                        ...
                        >
                            <div className="flex gap-3 sm:gap-6 items-start">
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 mb-2">
                                        <h3 className="font-bold text-white text-base sm:text-lg group-hover:text-primary transition-colors pr-6 sm:pr-8 truncate">
                                            {notif.title}
                                        </h3>
                                        <div className="flex items-center gap-1.5 text-white/40 text-[10px] sm:text-xs shrink-0">
                                            <Clock className="w-3 h-3" />
                                            <span>{notif.time}</span>
                                        </div>
                                    </div>
                                    <p className="text-white/60 text-xs sm:text-sm leading-relaxed mb-4 line-clamp-2 sm:line-clamp-none">
                                        {notif.description}
                                    </p>
                                    <div className="flex items-center gap-4">
                                        ...
                                        <button
                                            onClick={() => move.link && navigate(notif.link)}
                                            className="text-[10px] sm:text-xs font-bold text-white/40 hover:text-white"
                                        >
                                            View Details
                                        </button>
                                    </div>
                            ...
                            <button
                                ...
                                className="absolute top-3 right-3 sm:top-4 sm:right-4 p-1.5 text-white/20 hover:text-destructive hover:bg-destructive/10 rounded-lg transition-all opacity-100 sm:opacity-0 sm:group-hover:opacity-100"
                ...
                {notifications.length === 0 && (
                    <div className="text-center py-24 bg-white/5 rounded-3xl border border-dashed border-white/10 animate-in zoom-in-95 duration-300">

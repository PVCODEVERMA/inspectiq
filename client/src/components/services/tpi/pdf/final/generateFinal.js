...
export const generateFinal = (doc, data, currentY, contentWidth, primaryFont, checkPageBreak, drawTemplate, getBase64Image) => {
    const rows = [
        ["CLIENT:", data.client_name, "PO NUMBER:", data.po_number],
        ["DATE:", data.date, "VENDOR:", data.vendor_name],
        ["LOCATION:", data.location, "ITEM DESCRIPTION:", data.item_desc],
    ...
    // Final Inspection Checklist Table
    currentY = drawSectionHeader(doc, 'FINAL INSPECTION CHECKLIST', currentY, contentWidth, primaryFont);
    const items = Array.isArray(data.checklist) ? data.checklist : [];
    currentY = drawTable(doc,
        ['Inspection Item', 'Document Ref', 'Result', 'Remarks'],
        items.map(i => [i.item, i.document_ref, i.result, i.remarks]),

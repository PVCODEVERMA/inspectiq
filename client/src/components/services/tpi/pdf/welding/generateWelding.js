...
export const generateWelding = (doc, data, currentY, contentWidth, primaryFont, checkPageBreak, drawTemplate, getBase64Image) => {
    const rows = [
        ["CLIENT:", data.client_name, "PO NUMBER:", data.po_number],
        ["DATE:", data.date, "VENDOR:", data.vendor_name],
        ["LOCATION:", data.location, "WELDING PROCESS:", data.process],
        ["WELD JOINT ID:", data.weld_joint, "WPS REFERENCE:", data.wps_ref],
        ["WELDER ID / STAMP:", data.welder_id, "", ""]
    ];
    rows.forEach(row => {
    ...
    currentY = drawSectionHeader(doc, 'WELD INSPECTION CHECKLIST', currentY, contentWidth, primaryFont);
    const items = Array.isArray(data.inspection) ? data.inspection : [];
    currentY = drawTable(doc,
        ['Joint No', 'Visual Result', 'Dimensional Result', 'Remarks'],
        items.map(i => [i.joint_no, i.visual_result, i.dimension_result, i.remarks]),
